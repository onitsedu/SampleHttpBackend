...
public class LevelManagerImpl implements LevelManager {
	...
	public synchronized void addUserScore(UserScore score) {
		...
		if (level == null) {
			...
			DataSingleton.INSTANCE.getGameLevels().putIfAbsent(
					score.getLevelId(), level);
		}
		addScore(level, score);
	}

	public GameLevel getLevelRanking(Integer levelId)
	...
	private boolean addOrReplace(UserScore score,
			ConcurrentSkipListSet<UserScore> highScores) {
